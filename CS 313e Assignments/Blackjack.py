  ...
  # Description: run a game of blackjack, allowing players 1 - 6
...
class Player (object):
  ...
  def __str__ (self):
    ...
    return(player_hand + "- " + str(self.getPoints()) + " points")
...
class Dealer (Player):
  ...
  def __str__ (self):
    if self.show_one_card:
      ...
      return str(Player.__str__(self))
...
class Blackjack (object):
  ...
  def play (self, numPlayers):
    ...
    for i in range(numPlayers):
        if dealerPoints > 21 and self.Player[i].getPoints() <= 21:
          print('Player ' + str(i + 1) + ' wins')
        elif self.Players[i].getPoints() > 21:
          print ('Player ' + str(i + 1) +   ' loses')
	    
        elif dealerPoints > self.Players[i].getPoints() and dealerPoints <= 21:
          print ('Player ' + str(i + 1) +  ' loses')
        elif (dealerPoints < self.Players[i].getPoints() and self.Players[i].getPoints() <= 21):
          print ('Player ' + str(i + 1) + ' wins')
